// ============================================================
// Worker unico del proyecto: sirve el sitio estatico (public/)
// y gestiona la API (/api/*) contra la misma base D1.
// Fusionado desde toshinori-pwa/src/index.js para que el pipeline
// de build de AI Studio despliegue frontend + backend juntos.
// ============================================================
...
// Endpoints publicos (enroll, contact, attendance): permisivo, no exponen
// datos de terceros, solo reciben envios. Mismo origen ahora, pero se deja
// abierto por si en el futuro se llama desde otra herramienta externa.
...
// Endpoints del panel de admin: al ser ahora el MISMO origen que el sitio,
// no hace falta whitelisting de dominio para que el propio panel funcione
// (las peticiones same-origin no pasan por CORS). Por defecto NO se envia
// Access-Control-Allow-Origin => cualquier lectura cross-origin queda
// bloqueada por el navegador. Solo se habilita explicitamente si se define
// ADMIN_ALLOWED_ORIGIN (para casos futuros, ej. un admin en otro dominio).
function adminCorsHeaders(env) {
  ...
  if (env.ADMIN_ALLOWED_ORIGIN) {
    headers["Access-Control-Allow-Origin"] = env.ADMIN_ALLOWED_ORIGIN;
  }
  return headers;
}
...
// ---------- Autenticacion del panel de admin (JWT minimalista, Web Crypto) ----------
...
async function handleApiRequest(request, env, url) {
  ...
  try {
    // 1. Pre-inscripcion escolar (publico)
    ...
    // 2. Mensajes de contacto (publico)
    ...
    // 3. Control de asistencias de tutores (publico)
    ...
    // 4. Panel de administracion - login
    ...
    // 5. Panel de administracion - listado de inscripciones (protegido)
    ...
    // 6. Panel de administracion - listado de mensajes de contacto (protegido)
...
export default {
  async fetch(request, env, ctx) {
    ...
    // Enrutamiento explicito: /api/* va al manejador de la API; todo lo
    // demas se delega tal cual al binding de assets estaticos (mismo
    // comportamiento, incluido el fallback SPA de not_found_handling,
    // que ya tenia el proyecto antes de fusionar).
